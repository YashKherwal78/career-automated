...
def search_company_domain(company_name: str) -> str:
    ...
    if not company_name:
        return ""
    try:
        results = DDGS().text(f'"{company_name}" official website', max_results=5)
        if not results:
            return ""
...
def enrich_recruiter_email(
    ...
    if not company_domain and job_data.get("company_name"):
        ...
        company_domain = search_company_domain(job_data["company_name"])
