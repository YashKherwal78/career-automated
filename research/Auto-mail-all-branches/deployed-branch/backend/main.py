...
class PipelineRequest(BaseModel):
    ...
    mailmeteor_key: Optional[str] = ""
    salesql_key: Optional[str] = ""
...
async def parse_pdf(file: UploadFile = File(...)):
    try:
        ...
        return {"text": text, "pdf_base64": pdf_base64}
...
async def run_pipeline(req: PipelineRequest):
    ...
    best_email, source, all_contacts = enrich_recruiter_email(
        ...
        mailmeteor_api_key=req.mailmeteor_key or os.getenv("MAILMETEOR_API_KEY", ""),
        salesql_api_key=req.salesql_key or os.getenv("SALESQL_API_KEY", ""),
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if not shutil.which("pdflatex"):
                raise HTTPException(status_code=500, detail="pdflatex not found on server. Please install MacTeX or BasicTeX.")
