...
def extract_job_details(raw_text: str, api_keys: dict = None) -> dict:
    ...
    system_prompt = (
        ...
        "- company_website (extract official website URL if text contains it, OR heavily rely on your extensive pretraining knowledge to confidently state the company's real root domain)"
    )
    
    try:
        response_content = _run_with_fallback(
            api_keys=api_keys,
...
# ── Round-robin Provider State (in-memory) ──────────────────────────────────
_last_provider_idx = 0

def _get_active_providers(api_keys: dict) -> list[dict]:
    """Collect all configured API providers from request payload or env."""
    providers = []
    
    # 1. Groq
    groq_key = api_keys.get("groq_key") or os.getenv("GROQ_API_KEY")
    if not groq_key: 
        groq_key = os.getenv("GROK_API_1") or os.getenv("GROQ_API_1")
    if groq_key:
        providers.append({
            "name": "groq",
    ...
    # 2. Fireworks AI
    fireworks_key = api_keys.get("fireworks_key") or os.getenv("FIREWORKS_API_KEY")
    if fireworks_key:
        providers.append({
            "name": "fireworks",
    ...
    if not providers:
        raise ValueError("No LLM API keys found for Groq or Fireworks AI.")
    
    return providers

import requests

def _run_with_fallback(api_keys: dict, response_format=None, **kwargs) -> str:
    """Round-robin key rotation across [Groq, Fireworks] using direct REST.
    
    Returns:
        The content string directly.
    """
    global _last_provider_idx
    providers = _get_active_providers(api_keys)
    n = len(providers)
    
    start_idx = (_last_provider_idx + 1) % n
    last_error = None
    ...
    for attempt in range(n):
        ...
        try:
            ...
            if resp.ok and "choices" in data:
                ...
                logging.getLogger(__name__).info(f"API call succeeded on {provider['name']} ({idx+1}/{n})")
                return data["choices"][0]["message"]["content"]
            else:
                status = resp.status_code
                if status in (401, 402, 429, 500, 502, 503, 504):
                    logging.getLogger(__name__).warning(
                        f"API Error ({status}) on {provider['name']} ({idx+1}/{n}): {data}. Trying next..."
                    )
                    last_error = Exception(f"API Error {status}: {data}")
                else:
                    raise Exception(f"Fatal API Error on {provider['name']}: {data}")
                    
        except requests.exceptions.RequestException as e:
            logging.getLogger(__name__).warning(f"Connection error on {provider['name']} ({idx+1}/{n}): {e}. Trying next...")
            last_error = e
        except Exception as e:
            logging.getLogger(__name__).error(f"Unexpected error on {provider['name']} ({idx+1}/{n}): {e}. Trying next...")
            last_error = e

    if last_error:
        logging.getLogger(__name__).error("All LLM providers exhausted. Final failure.")
        raise last_error
    return ""
