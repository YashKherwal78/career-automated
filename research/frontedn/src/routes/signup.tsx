...
export const Route = createFileRoute("/signup")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Sign up for CareerAutomated. Free to start. Two minutes to set up. Cancel anytime." },
      { property: "og:title", content: "Create your account — CareerAutomated" },
      { property: "og:description", content: "Join CareerAutomated and get your time back." },
    ],
  }),
  component: SignUp,
});
    ...
    <div className="grid min-h-[calc(100vh-4rem)] lg:grid-cols-2">
      ...
      <div className="flex items-center justify-center px-6 py-16">
        <FadeIn className="w-full max-w-md">
          <div className="text-[11px] font-medium uppercase tracking-[0.18em] text-ink-soft">Get started</div>
          <h1 className="mt-3 font-display text-4xl leading-[1.05] tracking-tight text-ink md:text-5xl">
            Create your account
          </h1>
          <p className="mt-3 text-[15px] text-ink-soft">
            Already have one?{" "}
            <Link to="/signup" className="font-medium text-ink underline underline-offset-4 hover:text-[color:var(--peach-deep)]">
              Sign in
            </Link>
          </p>
...
function SocialButton({ provider, disabled }: { provider: "Google" | "GitHub"; disabled?: boolean }) {
  return (
    <button
      type="button"
      disabled={disabled}
      title={disabled ? "Coming soon" : undefined}
      className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border hairline bg-white text-sm font-medium text-ink transition hover:border-ink/30 disabled:cursor-not-allowed disabled:opacity-60"
    >
...
function GitHubIcon() {
  return (
    <svg viewBox="0 0 24 24" className="h-4 w-4" fill="currentColor">
      <path d="M12 .5C5.6.5.5 5.7.5 12.1c0 5.1 3.3 9.4 7.9 10.9.6.1.8-.3.8-.6v-2.1c-3.2.7-3.9-1.5-3.9-1.5-.5-1.3-1.3-1.7-1.3-1.7-1.1-.7.1-.7.1-.7 1.2.1 1.8 1.2 1.8 1.2 1 1.8 2.7 1.3 3.4 1 .1-.8.4-1.3.7-1.6-2.6-.3-5.3-1.3-5.3-5.7 0-1.3.5-2.3 1.2-3.2-.1-.3-.5-1.5.1-3.1 0 0 1-.3 3.2 1.2.9-.3 1.9-.4 2.9-.4s2 .1 2.9.4c2.2-1.5 3.2-1.2 3.2-1.2.6 1.6.2 2.8.1 3.1.7.8 1.2 1.9 1.2 3.2 0 4.4-2.7 5.4-5.3 5.7.4.4.8 1.1.8 2.2v3.3c0 .3.2.7.8.6 4.6-1.5 7.9-5.8 7.9-10.9C23.5 5.7 18.4.5 12 .5z" />
    </svg>
  );
}
