...
logger = setup_logger('wellfound_daily_cron')
import os
import time
...
class WellfoundDailyCron:
    ...
    def run(self):
        logger.info(f"[{datetime.now()}] Starting Wellfound Daily Cron (Limit: {self.limit})")
        ...
        if not jobs:
            logger.info("No jobs discovered. Exiting.")
            return
        ...
        logger.info(f"Selected Top {len(selected_jobs)} jobs for application out of {len(top_jobs)} passing jobs.")
        
        # 4. Apply
        for rank, job in enumerate(selected_jobs, 1):
            logger.info(f"\n--- Executing Job {rank}/{self.limit}: {job['title']} @ {job['company']} ---")
            logger.info(f"Opportunity Score: {job['opportunity_score']}")
            
            # Execute Application Flow
            handler = WellfoundHandler(job["url"])
            state = handler.execute()
            
            logger.info(f"Final State: {state.name}")
        ...
        logger.info(f"[{datetime.now()}] Wellfound Daily Cron Complete. Report saved to {report_path}")
