...
logger = setup_logger('inbox_checker')
import imaplib
import email
...
def check_inbox_for_replies():
    ...
    try:
        ...
        if status != "OK" or not messages[0]:
            logger.info("No new unseen messages.")
            mail.logout()
            return
        ...
        for num in messages[0].split():
            ...
            for response_part in data:
                if isinstance(response_part, tuple):
                    ...
                    logger.info(f"Found unseen email from: {sender_email}")
                    ...
                    if row:
                        ...
                        logger.info(f"Match found for HR of {company_name}!")
                        if current_status not in ["HR Replied", "Interview Scheduled"]:
                            ...
                            logger.info(f"Updated status for {company_name} to HR Replied.")
                    ...
                    if row:
                        company_name, current_status = row
                        logger.info(f"Match found for Founder of {company_name}!")
                        if current_status not in ["Founder Replied", "Interview Scheduled"]:
                            ...
                            logger.info(f"Updated status for {company_name} to Founder Replied.")
