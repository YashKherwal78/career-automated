...
logger = setup_logger('personio_adapter')
import os
...
from src.applications.handlers.personio import PersonioHandler
...
class PersonioAdapter(BaseAdapter):
    ...
    def apply(self, job: Dict[str, Any], resume_path: str, profile_manager: Any, test_mode: bool = False) -> ApplicationResult:
        logger.info(f"[PersonioAdapter] Launching browser for Job: {job.get('id')} - {job.get('company_name')}")
        ...
        with LaunchedBrowser() as lb:
            ...
            try:
                ...
                logger.info(f"[PersonioAdapter] Exception: {e}")
