...
logger = setup_logger('teamtailor_adapter')
import os
...
from src.applications.handlers.teamtailor import TeamTailorHandler
...
class TeamTailorAdapter(BaseAdapter):
    ...
    def apply(self, job: Dict[str, Any], resume_path: str, profile_manager: Any, test_mode: bool = False) -> ApplicationResult:
        logger.info(f"[TeamTailorAdapter] Launching browser for Job: {job.get('id')} - {job.get('company_name')}")
        ...
        with LaunchedBrowser() as lb:
            ...
            try:
                ...
                logger.info(f"[TeamTailorAdapter] Exception: {e}")
