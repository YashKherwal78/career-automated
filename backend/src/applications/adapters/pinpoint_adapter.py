...
logger = setup_logger('pinpoint_adapter')
import os
...
from src.applications.handlers.pinpoint import PinpointHandler
...
class PinpointAdapter(BaseAdapter):
    ...
    def apply(self, job: Dict[str, Any], resume_path: str, profile_manager: Any, test_mode: bool = False) -> ApplicationResult:
        logger.info(f"[PinpointAdapter] Launching browser for Job: {job.get('id')} - {job.get('company_name')}")
        ...
        with LaunchedBrowser() as lb:
            ...
            try:
                ...
                logger.info(f"[PinpointAdapter] Exception: {e}")
