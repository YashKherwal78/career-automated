...
logger = setup_logger('recruitee_adapter')
import os
...
from src.applications.handlers.recruitee import RecruiteeHandler
...
class RecruiteeAdapter(BaseAdapter):
    ...
    def apply(self, job: Dict[str, Any], resume_path: str, profile_manager: Any, test_mode: bool = False) -> ApplicationResult:
        logger.info(f"[RecruiteeAdapter] Launching browser for Job: {job.get('id')} - {job.get('company_name')}")
        ...
        with LaunchedBrowser() as lb:
            ...
            try:
                ...
                logger.info(f"[RecruiteeAdapter] Exception: {e}")
