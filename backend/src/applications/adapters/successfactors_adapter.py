...
logger = setup_logger('successfactors_adapter')
import os
...
from src.applications.handlers.successfactors import SuccessFactorsHandler
...
class SuccessFactorsAdapter(BaseAdapter):
    ...
    def apply(self, job: Dict[str, Any], resume_path: str, profile_manager: Any, test_mode: bool = False) -> ApplicationResult:
        logger.info(f"[SuccessFactorsAdapter] Launching browser for Job: {job.get('id')} - {job.get('company_name')}")
        ...
        with LaunchedBrowser() as lb:
            ...
            try:
                ...
                logger.info(f"[SuccessFactorsAdapter] Exception: {e}")
