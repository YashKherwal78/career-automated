...
logger = setup_logger('jazzhr_adapter')
import os
...
from src.applications.handlers.jazzhr import JazzHRHandler
...
class JazzHRAdapter(BaseAdapter):
    ...
    def apply(self, job: Dict[str, Any], resume_path: str, profile_manager: Any, test_mode: bool = False) -> ApplicationResult:
        logger.info(f"[JazzHRAdapter] Launching browser for Job: {job.get('id')} - {job.get('company_name')}")
        ...
        with LaunchedBrowser() as lb:
            ...
            try:
                ...
                logger.info(f"[JazzHRAdapter] Exception: {e}")
