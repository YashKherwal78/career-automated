...
logger = setup_logger('rippling_adapter')
import os
...
from src.applications.handlers.rippling import RipplingHandler
...
class RipplingAdapter(BaseAdapter):
    ...
    def apply(self, job: Dict[str, Any], resume_path: str, profile_manager: Any, test_mode: bool = False, user_id: str = None) -> ApplicationResult:
        logger.info(f"[RipplingAdapter] Launching browser for Job: {job.get('id')} - {job.get('company_name')}")
        ...
        with LaunchedBrowser() as lb:
            ...
            try:
                ...
                logger.info(f"[RipplingAdapter] Exception: {e}")
