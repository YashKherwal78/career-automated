...
logger = setup_logger('breezy_adapter')
import os
...
from src.applications.handlers.breezy import BreezyHandler
...
class BreezyAdapter(BaseAdapter):
    ...
    def apply(self, job: Dict[str, Any], resume_path: str, profile_manager: Any, test_mode: bool = False) -> ApplicationResult:
        logger.info(f"[BreezyAdapter] Launching browser for Job: {job.get('id')} - {job.get('company_name')}")
        ...
        with LaunchedBrowser() as lb:
            ...
            try:
                ...
                logger.info(f"[BreezyAdapter] Exception: {e}")
