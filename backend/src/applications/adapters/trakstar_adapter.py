...
logger = setup_logger('trakstar_adapter')
import os
...
class TrakstarAdapter(BaseAdapter):
    """Registered under the dispatcher's "recruiterbox" provider key —
    that's still the discovery pipeline's provider_id for these boards,
    even though the platform itself rebranded to Trakstar Hire."""
    ...
    def apply(self, job: Dict[str, Any], resume_path: str, profile_manager: Any, test_mode: bool = False) -> ApplicationResult:
        logger.info(f"[TrakstarAdapter] Launching browser for Job: {job.get('id')} - {job.get('company_name')}")
        ...
        with LaunchedBrowser() as lb:
            ...
            try:
                ...
                logger.info(f"[TrakstarAdapter] Exception: {e}")
