...
logger = logging.getLogger("FreshteamConnector")

class FreshteamConnector(Connector):
    ...
    async def sync(self, board: Board, http_client: HttpClient) -> AsyncIterator[RawJob]:
        ...
        url = f"https://{tenant}.freshteam.com/jobs/api"
            ...
            if result.status_code == 200:
                ...
                for job in jobs:
                    if isinstance(job, dict):
                        yield RawJob(company_id=board.company_id, provider="freshteam", board_identity=board.identity, payload=job)
        except Exception as e:
            logger.warning(f"FreshteamConnector[{tenant}] sync failed: {e}")

ConnectorRegistry.register('freshteam', 'JSON', 50, FreshteamConnector)
